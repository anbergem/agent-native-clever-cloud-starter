...
function runModelEvals(args = ["--json"]) {
  ...
  // `detectEngineFromEnv()` walks the whole registry and reaches `ai-sdk:ollama`,
  // which needs no API key and therefore looks available — then every eval waits out
  // a connect timeout against a local Ollama that is not running. That took this file
  // from 2 seconds to 910 (DISCREPANCIES.md, 2026-09-24), and pinning `AGENT_ENGINE`
  // instead is not enough: an AI-SDK engine with no key retries against the real API
  // and still costs a minute per eval. Narrowing the registry is what makes
  // `resolveEngine` refuse before any request exists, which is the refusal this test
  // is actually about.
  //
  // The value is a comma-separated list despite the framework documenting the option as
  // `["ai-sdk:openai"]`: passing that JSON spelling is read as one engine name and
  // refused with "names unknown built-in engine(s)".
  const env = {
    ...process.env,
    RUN_MODEL_EVALS: "1",
