...
for `GET /` without running a loader. The redirect to `/jobs` is therefore client-side
(`<Navigate to="/jobs" replace />` in `app/routes/_index.tsx`); a loader `redirect()` there
...
sequenceDiagram
  ...
  participant Repo as JobRepository (SQL)
