 ...
 *
 * PostgreSQL and the local SQLite file both expose `transaction`, so today this seam
 * costs nothing — but it is the reason the move off Cloudflare touched no repository
 * (T28). D1 exposed `atomicBatch` and no interactive transactions, and `runAtomic`
 * absorbed that difference: repositories build a list of statements and never learn
 * which runtime applied them. The `atomicBatch` branch is kept for the same reason it
 * was written, which is that the next runtime may not look like this one either.
 *
